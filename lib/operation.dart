...
class OperationPocheRonde extends Operation {
  ...
  OperationPocheRonde(
      {required super.OriginZ, required super.OriginY, required super.OriginX, super.label, required this.ParamD, required this.ParamC, required this.ParamDf, required this.ParamAP});
  ...
  Future<void> construct() async {
    ...
    for (double j = ParamAP; j < ParamC; j += ParamAP) {
      ...
      for (double i = (ParamDf / 2); i < ((ParamD/2) - (ParamDf / 2));
      i += (ParamDf / 2)) {
        trajectoires.add('G1 X${(OriginX-i).toString()}');
        trajectoires.add(
            'G2 X${(OriginX - i).toString()}  I$i');
      }
      trajectoires.add('G1 X${((OriginX - (ParamD/2))+(ParamDf/2))
          .toString()}');
      trajectoires.add(
          'G2 X+${((OriginX - (ParamD/2))+(ParamDf/2))
              .toString()} I${((ParamD/2)-(ParamDf/2))
              .toString()}');
    }
